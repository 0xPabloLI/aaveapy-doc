...
来源：[Aave V3 ValidationLogic.sol 源码分析](https://github.com/aave-dao/aave-v3-origin/blob/96c2e96/src/contracts/protocol/libraries/logic/ValidationLogic.sol)（commit [`96c2e96`](https://github.com/aave-dao/aave-v3-origin/tree/96c2e96)），当前分析版本：**V3.6.0**（`package.json: "version": "3.6.0"`）

### 四个标志总览

V3 的 `ReserveConfiguration.getFlags()` 返回四个独立标志（`[合约源码证实]`）：

```solidity
// [ReserveConfiguration.sol](https://github.com/aave-dao/aave-v3-origin/blob/96c2e96/src/contracts/protocol/libraries/configuration/ReserveConfiguration.sol)
...
**校验规则**：几乎所有函数都强制要求 `isActive == true`，唯一例外是 `validateTransfer()`（aToken 转账），它只检查 `isPaused`（`[合约源码证实]` [ValidationLogic.sol L486-L488](https://github.com/aave-dao/aave-v3-origin/blob/96c2e96/src/contracts/protocol/libraries/logic/ValidationLogic.sol#L486-L488)）。

### `isFrozen`（冻结）

管理权限：Risk Admin / Pool Admin

**校验源码**（`[合约源码证实]` [ValidationLogic.sol](https://github.com/aave-dao/aave-v3-origin/blob/96c2e96/src/contracts/protocol/libraries/logic/ValidationLogic.sol)）：
- `validateSupply()` [L54-L65](https://github.com/aave-dao/aave-v3-origin/blob/96c2e96/src/contracts/protocol/libraries/logic/ValidationLogic.sol#L54-L65) — 完整解构 `(isActive, isFrozen, , isPaused)` → `require(!isFrozen)`
- `validateBorrow()` [L124-L152](https://github.com/aave-dao/aave-v3-origin/blob/96c2e96/src/contracts/protocol/libraries/logic/ValidationLogic.sol#L124-L152) — 完整解构 `(isActive, isFrozen, borrowingEnabled, isPaused)` → `require(!isFrozen)`
- `validateSetUseReserveAsCollateral()` [L239-L244](https://github.com/aave-dao/aave-v3-origin/blob/96c2e96/src/contracts/protocol/libraries/logic/ValidationLogic.sol#L239-L244) — **故意跳过**：`(isActive, , , isPaused)`（两个空逗号跳过 isFrozen 和 borrowingEnabled）
...
> `[合约源码证实：PoolConfigurator.setReserveFreeze() 在 freeze=true 时调用 _setReserveLtvzero(asset, true)](https://github.com/aave-dao/aave-v3-origin/blob/96c2e96/src/contracts/protocol/pool/PoolConfigurator.sol#L208-L220)`
...
基于对 `ValidationLogic.sol` 全部 7 处 `getFlags()` 调用的逐行分析（`[合约源码证实]` [ValidationLogic.sol](https://github.com/aave-dao/aave-v3-origin/blob/96c2e96/src/contracts/protocol/libraries/logic/ValidationLogic.sol)）：
...
来源：[aave-v4 源码](https://github.com/aave/aave-v4)（commit [`ef3c7ad`](https://github.com/aave/aave-v4/tree/ef3c7ad7)），基于对 `Spoke.sol`、`LiquidationLogic.sol`、`Hub.sol`、`ReserveFlagsMap.sol`、`SpokeConfigurator.sol` 的逐行分析。

V4 采用 **Hub & Spoke** 双层架构，标志分布在两个层级，各自独立控制。

### Hub 层面（per spoke per asset）

Hub 的 `SpokeData` 结构体包含两个独立的布尔标志（**注意：Hub 层没有 frozen/paused 概念**）（`[合约源码证实]` [IHub.sol L77-L89](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/hub/interfaces/IHub.sol#L77-L89)）：
...
> `active` 和 `halted` 是**两个独立标志**，**并非互补关系**。一个 Spoke 可以同时是 `active=true, halted=true`。`[合约源码证实]`：`deactivateSpoke` 和 `haltSpoke` 为两个独立 setter 函数，互不联动（[HubConfigurator.sol L259-L283](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/hub/HubConfigurator.sol#L259-L283)）。

**Hub 校验逻辑**（`[合约源码证实]` [Hub.sol L820-L868](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/hub/Hub.sol#L820-L868)）：
...
Spoke 的 `ReserveFlagsMap` 用一个 `uint8` 位图存储四个独立标志（`[合约源码证实]` [ReserveFlagsMap.sol L10-L18](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/libraries/ReserveFlagsMap.sol#L10-L18)）：
...
> **V4 没有 `isActive` 标志在 Spoke 层**——与 V3 不同，Spoke reserve 的「是否存在」通过 `_reserves.get(reserveId)`（检查 `hub != address(0)`）或 `reserveId < _reserveCount` 隐式检查，不需要单独的 active 标志。（`[合约源码证实]` [ReserveFlagsMap.sol L10-L18](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/libraries/ReserveFlagsMap.sol#L10-L18) 无 ACTIVE_MASK；[ISpoke.sol L61-L68](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/interfaces/ISpoke.sol#L61-L68) ReserveConfig 无 active 字段）
...
> `[架构推断，无直接代码证据]`：基于 V4 DynamicReserveConfig 版本化机制推断，未实际审查 ConfigEngine 中 freeze 操作是否联动 collateralFactor

#### `paused`（暂停）

管理权限：通过 `SpokeConfigurator.updatePaused()` / `pauseReserve()` / `pauseAllReserves()` 设置（`restricted` 权限）

**最严厉的运行时开关**。所有 Spoke 用户操作的 validate 函数都以 `require(!flags.paused())` 开头（`[合约源码证实]` [Spoke.sol L862-L891](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/Spoke.sol#L862-L891)）。
...
> 源码确认：`_validateLiquidationCall()` 对**双方** reserve（抵押品和债务）都检查 `!paused()`。（`[合约源码证实]` [LiquidationLogic.sol L533-L558](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/libraries/LiquidationLogic.sol#L533-L558)）
...
> rsETH 因安全事故被 freeze；WETH 已有存借活动后因同一事件被 freeze。
> 两者均属于 LRT（Liquid Restaking Token）类别，据观测是目前唯一 frozen 的 LRT spoke `[实测数据：v4-raw-sdk-response.json]`。
...
> （`[合约源码证实]` [ValidationLogic.sol L239-L244](https://github.com/aave-dao/aave-v3-origin/blob/96c2e96/src/contracts/protocol/libraries/logic/ValidationLogic.sol#L239-L244)）。
...
| 维度              | V3                                                | V4                                                |
| --------------- | ------------------------------------------------- | ------------------------------------------------- |
| 冻结对 HF 影响       | LTV 自动设为 0，Liquidation Threshold 不变 → HF 不受影响 [^1]     | collateralFactor 通过 DynamicReserveConfig 独立管理，需单独操作 [^2] |
...
> [^1]: `[合约源码证实]` [PoolConfigurator.sol L208-L220](https://github.com/aave-dao/aave-v3-origin/blob/96c2e96/src/contracts/protocol/pool/PoolConfigurator.sol#L208-L220)：`setReserveFreeze()` 在 `freeze=true` 时调用 `_setReserveLtvzero(asset, true)`
>
> [^2]: `[架构推断，无直接代码证据]` V4 使用 `DynamicReserveConfig` 版本化机制，frozen 不自动联动 collateralFactor，需治理单独操作

### 3.7 Hub active/halted 与 Spoke frozen/paused 的级联关系

**核心结论：两套状态完全解耦，不存在协议层的自动级联。**

> `[合约源码证实]`：`HubConfigurator` 的四个 halt/deactivate 函数体中仅调用 `targetHub.updateSpokeConfig()`——修改 Hub 自身存储，无任何对 Spoke 合约的 external call：
> - [deactivateAsset L135-L145](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/hub/HubConfigurator.sol#L135-L145)：循环设 `config.active = false` + `updateSpokeConfig`
> - [haltAsset L147-L157](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/hub/HubConfigurator.sol#L147-L157)：循环设 `config.halted = true` + `updateSpokeConfig`
> - [deactivateSpoke L259-L270](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/hub/HubConfigurator.sol#L259-L270)：同上，per-spoke 循环
> - [haltSpoke L272-L283](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/hub/HubConfigurator.sol#L272-L283)：同上，per-spoke 循环
...
### 3.8 Hub + Spoke 联合门控矩阵

V4 用户操作需**同时通过两层校验**才能执行。完整路径：

```
用户 → Spoke.validate(!paused, !frozen, ...) → Hub.validate(!active, !halted) → 执行
```

任一层拒绝即 revert，不存在"绕过 Hub 直达"的路径（Spoke 所有流动性操作都调用 Hub）（`[合约源码证实]` [Spoke.sol](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/Spoke.sol) supply/withdraw/borrow/repay 均调用 `HUB.add/remove/draw/restore`）。

#### 以标志为基准（每个标志关闭时，哪些操作被阻止）

| 标志 | 层级 | supply | withdraw | borrow | repay | setCollateral 启用 | setCollateral 禁用 | liquidationCall | liquidation + receiveShares |
|---|---|---|---|---|---|---|---|---|---|
| **`paused = true`** | Spoke | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ |
| **`frozen = true`** | Spoke | ❌ | ✅ | ❌ | ✅ | ❌ | ✅ | ✅ | ❌ |
| **`borrowable = false`** | Spoke | ✅ | ✅ | ❌ | ✅ | ✅ | ✅ | ✅ | ✅ |
| **`active = false`** | Hub | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ❌ | ❌ |
| **`halted = true`** | Hub | ❌ | ❌ | ❌ | ❌ | ✅ | ✅ | ✅ | ✅ |

> ❌ = 该标志处于限制状态时阻止此操作；✅ = 不阻止（操作仍可执行，但需其他标志也放行）
>
> - `paused=true` 是 Spoke 层最严厉开关，禁止全部操作
> - `frozen=true` 阻止新流入（supply/borrow/启用抵押品）但允许退出（withdraw/repay/禁用抵押品/被清算）
> - `borrowable=false` 仅阻止 borrow
> - `active=false` 是 Hub 层总开关，阻止所有需与 Hub 交互的操作；setCollateral 不经 Hub，不受影响
> - `halted=true` 阻止流动性变更（supply/withdraw/borrow/repay），但清算和 setCollateral 不受影响

#### 各操作的可执行条件（所有标志取交集）

| 操作 | 可执行条件 | 经 Hub? |
|---|---|---|
| supply | `!paused ∧ !frozen ∧ active ∧ !halted` | ✅ Hub.add |
| withdraw | `!paused ∧ active ∧ !halted` | ✅ Hub.remove |
| borrow | `!paused ∧ !frozen ∧ borrowable ∧ active ∧ !halted` | ✅ Hub.draw |
| repay | `!paused ∧ active ∧ !halted` | ✅ Hub.restore |
| setCollateral 启用 | `!paused ∧ !frozen` | ❌ 仅 Spoke 内部 |
| setCollateral 禁用 | `!paused` | ❌ 仅 Spoke 内部 |
| liquidationCall | `!paused ∧ active` | ✅ Hub.remove（不清算人 halted） |
| liquidation + receiveShares | `!paused ∧ !frozen ∧ receiveSharesEnabled ∧ active` | ✅ Hub.remove |

> **层级优先级**：`paused` > `frozen` > `borrowable`（Spoke 层）；`!active` > `halted`（Hub 层）。两层无优先级关系，必须同时满足。

**TokenizationSpoke 的额外穿透**（§3.7 已述，`[合约源码证实]` [TokenizationSpoke.sol L267-L270](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/TokenizationSpoke.sol#L267-L270)）：TokenizationSpoke 不走 Spoke 的 paused/frozen 门控，而是直接读 Hub `SpokeConfig`——当 `!active ∨ halted` 时，ERC4626 vault 的 `maxDeposit`/`maxWithdraw` 返回 0，等效于阻止操作但不 revert。

### 3.9 Spoke 与 TokenizationSpoke：同一 Asset 上的两种并列 Spoke
...
**实际部署规模**（Ethereum Mainnet，aave-address-book v4.49.0+）（`[实测数据：aave-address-book v4.49.0, AaveV4Ethereum.SPOKES/TOKENIZATION_SPOKES 数组长度]`）：
- Spoke：11 个（`AaveV4Ethereum.SPOKES`）
- TokenizationSpoke：31 个（`AaveV4Ethereum.TOKENIZATION_SPOKES`）
- SDK 暴露：仅 Spoke（GraphQL `Reserve.spoke`），TokenizationSpoke 不暴露（`[实测数据：V4 SDK GraphQL schema 中 Reserve 实体无 tokenizationSpoke 字段]`）
...
| Inactive + *    | Inactive 最高优先级  | Inactive 样式    | inactive 包含所有操作限制 |
...
> Paused 和 Inactive 共用 amber 背景色（最高优先级）。
...
## 五、合约标志 → SDK JSON 响应字段映射

### V3 vs V4 SDK 字段对比

| 标志 | V3 SDK（`@aave/client@0.6.1`） | V4 SDK（`@aave/client-v4@4.2.0`） |
|---|---|---|
| `isFrozen` / `frozen` | `reserve.isFrozen: boolean` ✅ | `reserve.status.frozen: boolean` ✅ |
| `isPaused` / `paused` | `reserve.isPaused: boolean` ✅ | `reserve.status.paused: boolean` ✅ |
| `isActive` / `active` | **❌ 不暴露**（V3 SDK reserve 级无此字段） | `reserve.status.active: boolean` ✅（indexer 派生） |

> V3 合约层有 `isActive` bit（`ReserveConfiguration` 位图 + `ReserveActive` 事件），但 V3 SDK **在 reserve 对象上不暴露此字段**。
> V4 合约层 Reserve 级无 `active` bit（§五下文详述），但 V4 SDK 的 `ReserveStatus` **暴露了 indexer 派生的 `active`**。
...
| 字段 | 层级 | 链上存在? | 含义 | 依据 |
|---|---|---|---|---|
| `Reserve.status.active` | Spoke Reserve | ❌ 链上无此 bit | reserve 是否可用 | 合约源码证实无 bit；推测为 indexer 派生（无直接证据） |
| `HubSpokeConfig.active` | Hub-Spoke | ✅ `SpokeData.active` | Spoke 是否被 Hub 允许执行任何操作 | 合约源码 |

#### 链上证据：Reserve 级别不存在 `active` bit

V4 Spoke 合约的 `ReserveFlagsMap.sol` 仅定义四个位掩码，**无 ACTIVE_MASK**：

```solidity
// [ReserveFlagsMap.sol L10-L18](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/libraries/ReserveFlagsMap.sol#L10-L18)
...
`ISpoke.sol` 的 `ReserveConfig` 结构体同样不含 `active` 字段：

```solidity
// [ISpoke.sol L61-L68](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/interfaces/ISpoke.sol#L61-L68)
...
对比 Hub 层的 `SpokeData`，`active` 是 **Spoke 级别**（非 Reserve 级别）的链上存储字段：

```solidity
// [IHub.sol L77-L89](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/hub/interfaces/IHub.sol#L77-L89)
struct SpokeData {
    uint120 drawnShares;
    ...
    bool active;     // ← Spoke 级别，不是 reserve 级别
    bool halted;     // ← Spoke 级别
    uint200 deficitRay;
}
```
...
```solidity
// [Spoke.sol L862-L891](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/Spoke.sol#L862-L891)
function _validateSupply(ReserveFlags flags) internal pure {
    require(!flags.paused(), ReservePaused());
    require(!flags.frozen(), ReserveFrozen());
}
...
```solidity
// [LiquidationLogic.sol L533-L558](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/libraries/LiquidationLogic.sol#L533-L558)
function _validateLiquidationCall(...) internal pure {
    require(!params.collateralReserveFlags.paused() && !params.debtReserveFlags.paused(), ...);
    // ... 其他校验 ...
    if (params.receiveShares) {
        require(!params.collateralReserveFlags.frozen() &&
            params.collateralReserveFlags.receiveSharesEnabled(),
            ISpoke.CannotReceiveShares());
    }
}
...
```solidity
// [Hub.sol L820-L868](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/hub/Hub.sol#L820-L868)
function _validateAdd(...) internal view {
    require(spoke.active, SpokeNotActive());
    require(!spoke.halted, SpokeHalted());
}
function _validateRemove(...) internal view {
    require(spoke.active, SpokeNotActive());
    require(!spoke.halted, SpokeHalted());
}
function _validateDraw(...) internal view {
    require(spoke.active, SpokeNotActive());
    require(!spoke.halted, SpokeHalted());
}
function _validateRestore(...) internal view {
    require(spoke.active, SpokeNotActive());
    require(!spoke.halted, SpokeHalted());
}
```

TokenizationSpoke 的 Hub 状态穿透（代码依据）：

```solidity
// [TokenizationSpoke.sol L267-L270](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/TokenizationSpoke.sol#L267-L270)
...
#### SDK 层：`ReserveStatus.active` 的来源推测

GraphQL schema 暴露了 `active` 字段，但链上 Reserve 级无此 bit：

```graphql
# [schema.graphql L1806-L1810](https://github.com/aave/aave-v4-sdk/blob/79f7e642/packages/graphql/schema.graphql#L1806-L1810)
type ReserveStatus {
  frozen: Boolean!
  paused: Boolean!
  active: Boolean!   # ← 链上无对应 bit，推测由 indexer 派生
}
```

SDK 客户端仅消费该字段，不负责计算逻辑：

```typescript
// [reserve.ts L43-L51](https://github.com/aave/aave-v4-sdk/blob/79f7e642/packages/graphql/src/fragments/reserve.ts#L43-L51)
...
> **推测为 indexer 派生，但无直接证据**：
> - Aave 官方未公开 V4 subgraph/indexer 源码（GitHub `org:aave` 无 `v4-subgraph`/`v4-indexer` 仓库）
> - `@aave/client-v4@4.2.0` 客户端仅消费字段，不含计算逻辑
> - 链上 Reserve 级无 active bit（已由合约源码证实）
> - 因此 `status.active` 只可能来自 GraphQL 服务端（indexer/API 层）的派生，但**派生公式和实现均未公开，无法确认**

#### 实测数据验证

`data/debug/v4-raw-sdk-response.json`（63 个 reserve）的组合分布：

| `active` | `frozen` | `paused` | 数量 | 占比 | 依据 |
|----------|----------|----------|------|------|------|
| `true` | `false` | `false` | 61 | 96.8% | 实测 |
| `true` | `true` | `false` | 2 | 3.2% | 实测（Kelp spoke WETH/rsETH） |
| `false` | * | * | 0 | — | 未观测到 |
| * | * | `true` | 0 | — | 未观测到 |

`frozen=true, paused=false` 的 reserve（Kelp spoke WETH/rsETH）其 `status.active` 仍为 `true`，
**否证 `active = !paused && !frozen`**。

**猜测的派生公式**：`active = !paused`（仅看 paused，不看 frozen）。

> **猜测依据**（全部为间接证据，无代码直接验证）：
> 1. 链上 Reserve 级无 active bit → `status.active` 必然是某层派生
> 2. 实测数据：当前所有 63 个 reserve 的观测组合均满足 `active = !paused`
> 3. 实测数据：`frozen=T,paused=F` 时 `active=T`，否证 `active = !paused && !frozen`
> 4. **局限**：未观测到 `paused=T` 的 reserve，无法确认 `paused=T` 时 `active` 是否为 `F`
> 5. Aave 未公开 indexer 源码，公式无法从代码确认

#### V3 对比：`active` 是链上事件直接映射

V3 合约发出 `ReserveActive(address asset, bool active)` 事件，subgraph 直接取值（代码依据）：

```typescript
// [v3.ts L186-L190](https://github.com/aave/protocol-subgraphs/blob/43ab69e1/src/mapping/lending-pool-configurator/v3.ts#L186-L190)
export function handleReserveActivated(event: ReserveActive): void {
  let reserve = getOrInitReserve(event.params.asset, event);
  reserve.isActive = event.params.active;  // ← 直接从事件参数取值
  saveReserve(reserve, event);
}
```

V3 subgraph schema 中 `Reserve.isActive` 是独立实体字段（代码依据）：

```graphql
# [v3.schema.graphql L544](https://github.com/aave/protocol-subgraphs/blob/43ab69e1/schemas/v3.schema.graphql#L544)
isActive: Boolean! # defalt false
```

V3 中 `active` 是 `ReserveConfiguration` 位图的一部分，是链上真实标志，subgraph 从合约事件直接映射。V4 在 Reserve 级别取消了这个 bit，由 indexer 派生填补。

#### 依据分类汇总

| 结论 | 依据类型 | 来源 |
|---|---|---|
| V4 Reserve 级无 `active` bit | 合约源码 | [ReserveFlagsMap.sol](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/libraries/ReserveFlagsMap.sol#L10-L18)、[ISpoke.sol](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/interfaces/ISpoke.sol#L61-L68) |
| Hub `SpokeData.active` 是链上存储 | 合约源码 | [IHub.sol L77-L89](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/hub/interfaces/IHub.sol#L77-L89) |
| Spoke paused/frozen/borrowable 校验逻辑 | 合约源码 | [Spoke.sol L862-L891](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/Spoke.sol#L862-L891) |
| Hub active/halted 校验逻辑 | 合约源码 | [Hub.sol L820-L868](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/hub/Hub.sol#L820-L868) |
| 清算 receiveShares 校验逻辑 | 合约源码 | [LiquidationLogic.sol L533-L558](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/libraries/LiquidationLogic.sol#L533-L558) |
| TokenizationSpoke Hub 状态穿透 | 合约源码 | [TokenizationSpoke.sol L267-L270](https://github.com/aave/aave-v4/blob/ef3c7ad7/src/spoke/TokenizationSpoke.sol#L267-L270) |
| SDK `ReserveStatus.active` 来源推测 | 间接推测 | 链上无 bit + SDK 仅消费 + 官方未公开 indexer |
| `active = !paused && !frozen` 被否证 | 实测数据 | `data/debug/v4-raw-sdk-response.json`（Kelp WETH/rsETH: `active=T,frozen=T,paused=F`） |
| `active = !paused` 猜测 | 实测猜测 | 同上（所有 63 个 reserve 均满足，但未观测到 `paused=T` 组合，无代码验证） |
| V3 `isActive` 是链上事件映射 | subgraph 源码 | [v3.ts L186-L190](https://github.com/aave/protocol-subgraphs/blob/43ab69e1/src/mapping/lending-pool-configurator/v3.ts#L186-L190)、[v3.schema.graphql L544](https://github.com/aave/protocol-subgraphs/blob/43ab69e1/schemas/v3.schema.graphql#L544) |

**结论**：前端判断 reserve 可用性应直接使用 `canSupply`/`canBorrow`/`canUseAsCollateral` 等 SDK 计算字段，不应自行组合 `active && !frozen`。Hub 层的 `HubSpokeConfig.active` 通过 GraphQL `hubSpokeConfigs` 查询获取（`@aave/client-v4@4.2.0` 不含此 action，需直调或升级）。
