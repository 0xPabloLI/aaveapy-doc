...
任一层拒绝即 revert，不存在"绕过 Hub 直达"的路径（Spoke 所有流动性操作都调用 Hub）。
...
**TokenizationSpoke 的额外穿透**（§3.7 已述）：TokenizationSpoke 不走 Spoke 的 paused/frozen 门控，而是直接读 Hub `SpokeConfig`——当 `!active ∨ halted` 时，ERC4626 vault 的 `maxDeposit`/`maxWithdraw` 返回 0，等效于阻止操作但不 revert。
...
| 字段 | 层级 | 存储 | 含义 | 依据 |
|---|---|---|---|---|
| `Reserve.status.active` | Spoke Reserve | indexer 派生（链上无此 bit） | reserve 是否已配置且未被 paused | 实测推断 |
| `HubSpokeConfig.active` | Hub-Spoke | **链上存储** `SpokeData.active` | Spoke 是否被 Hub 允许执行任何操作 | 合约源码 |
...
#### SDK 层：`ReserveStatus.active` 是 indexer 派生字段

GraphQL schema 定义了三个同级布尔字段，但 `active` 无链上 bit 对应：
...
type ReserveStatus {
  frozen: Boolean!
  paused: Boolean!
  active: Boolean!   # ← indexer 派生，非链上映射
}
```
...
```typescript
// [v4-fetcher.ts L156-L158,L205-L207](../../../packages/aave-fetcher/src/v4-fetcher.ts#L156)
...
**推断的派生公式**：`active = !paused`（仅看 paused，不看 frozen）。

> 此公式为**实测数据推断**，非代码验证——Aave 官方未公开 V4 subgraph/indexer 源码，
> `@aave/client-v4@4.2.0` 客户端仅消费 `status.active` 字段不负责计算。
> 推断依据：当前所有观测组合（`active=T,frozen=F,paused=F` 61 个 + `active=T,frozen=T,paused=F` 2 个）
> 均满足 `active = !paused`。需观测到 `paused=T` 的 reserve 才能最终确认。
...
| SDK `ReserveStatus.active` 是派生字段 | SDK 源码 | [schema.graphql L1806](https://github.com/aave/aave-v4-sdk/blob/79f7e642/packages/graphql/schema.graphql#L1806-L1810)、[reserve.ts L43](https://github.com/aave/aave-v4-sdk/blob/79f7e642/packages/graphql/src/fragments/reserve.ts#L43-L51) |
| `active = !paused && !frozen` 被否证 | 实测数据 | `data/debug/v4-raw-sdk-response.json`（Kelp WETH/rsETH: `active=T,frozen=T,paused=F`） |
| `active = !paused` 推断 | 实测推断 | 同上（所有 63 个 reserve 均满足，但未观测到 `paused=T` 组合） |
