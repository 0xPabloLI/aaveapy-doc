...
| 属性 | 说明 |
|------|------|
| 存储（V3） | `reserve.deficit`（per reserve，单层存储） |
| 存储（V4） | 双层：`Asset.deficitRay`（Hub 聚合）+ `SpokeData.deficitRay`（per spoke），满足 `Asset.deficitRay = Σ SpokeData.deficitRay` |
| 单位（V3） | underlying token units |
| 单位（V4） | RAY precision（1e27） |
| 数据获取（V3） | `Pool.getReserveData(asset)` → `deficit` 字段；或 `UiPoolDataProviderV3.getReservesHumanized()` |
| 数据获取（V4） | `Hub.getSpokeDeficitRay(assetId, spoke)`（per-spoke 分量，语义对齐 V3 reserve.deficit）；**后端已实现**——通过 `onchainDataService.ts` + Multicall3 批量 RPC 获取（~94 serial → ~16 batch） |
...
```
用户抵押品价值 < 用户债务
→ 债务 token 销毁（totalVariableDebt 减少）
→ 清算后差额 = 债务 - 实际收回抵押品价值
...
| 版本 | 清算后债务处理 | deficit 记账 |
|------|---------------|-------------|
| V3 | `variableDebtToken.burn()` 销毁，`totalVariableDebt` 减少 | 差额单独存入 `reserve.deficit`，**不影响** `totalVariableDebt` 状态 |
| V4 | `drawnShares` 减少，债务份额核销 | 差额等量计入 `deficitRay`，满足 `drawnShares↓ = deficitRay↑` |

V3 将"有效债务"和"坏账"分开存储，V4 通过 shares-based accounting 统一体现在份额变化中。
...
**经济学意义**：deficit 对应的债务没有借款人支付利息。原借款人已被清算，债务 token 已销毁，但抵押品不足以覆盖债务，差额成为坏账。供应者通过降低的 supply APY 承担这部分资金损失。
...
| 合约 | 函数 | 获取内容 |
|------|------|----------|
| `Hub` | `getAssetDeficitRay(assetId)` | Hub 聚合 deficit（该 asset 所有 spoke 的坏账总额） |
| `Hub` | `getSpokeDeficitRay(assetId, spoke)` | 单个 spoke 的 deficit 分量 |
| `Hub` | `getAsset(assetId)` | 完整 Asset 结构（含 `deficitRay` 字段） |
| `Hub` | `getAssetDrawnShares(assetId)` | 当前总借款 shares（清算后会减少） |
| `Hub` | `getAssetDrawnIndex(assetId)` | 债务指数 |
| `Hub` | `getAssetDrawnRate(assetId)` | 当前借款利率 |

**注意**：V4 的 deficit 以 RAY 精度存储，使用时需 `deficitRay.fromRayUp()` 转换为 asset units。
...
> **官方文档**：[Aave V4 Whitepaper §4](https://github.com/aave/aave-v4/blob/main/docs/WHITEPAPER.md)、[ERC-4626 Tokenized Vault Standard](https://eips.ethereum.org/EIPS/eip-4626)
>
> **Shares-based accounting**：用"份额"（shares）而非"绝对金额"记账，余额 = shares × price/index。优势：① 避免每次操作更新所有用户余额（gas 优化）② 与 ERC-4626 vault 标准一致，支持组合性 ③ 供应侧无需维护 liquidityIndex，简化状态管理。
...
**证据**：不变量由合约逻辑保证——`reportDeficit` 和 `eliminateDeficit` 对 Hub 聚合和 Spoke 分量 pair-wise 同步增减，不存在只改一边的路径：
- `reportDeficit`：`asset.deficitRay += amount; spoke.deficitRay += amount;`（`src/hub/Hub.sol:320-323`）
- `eliminateDeficit`：`asset.deficitRay -= amount; coveredSpoke.deficitRay -= amount;`（`src/hub/Hub.sol` eliminateDeficit 函数）
